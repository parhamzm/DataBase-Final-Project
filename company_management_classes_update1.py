...
class Distributer(Person):
    def __init__(self, dis_id=None, account_id=None):
...
""".format(self.project_id, self.project_name, self.start_date_proj, self.end_date_proj)
    ...
    def edit_Projects(self):
        ...
        print(a)
        if a == None:
            print("--->>>Error : There is No Projects with that Info. try Again --->")
            input_try = input("if you want to try Again Enter <<<1>>> : ")
            if input_try == '1':
                # print("hello")
                self.edit_Projects()
...
class Produce:
    def __init__(self):
        #pass
        def __init__(self, ProduceID=None, ProductID=None, ProduceDate=None):
            self.product_name = ProduceID
            self.product_id = ProductID
            self.price = ProduceDate
    
    def add_Produce(self):
        pass
        print("--- >>>> Welcom to the Add New 'Produce' Section <<<< ---")
        self.Produce_ID = input("please Enter the ID of the Produce : ")
        while Produce_ID == "":
            print("<<<Attention>>> : Project ID can't be Empty!!")
            self.Produce_ID = input("---> please Enter the ID of the Produce :")
            self.Product_ID = input("please Enter the ID of the Product : ")
            self.Produce_Date = input("please Enter the Date of the Produce : ")

        cur = conn.cursor()
        st = """
INSERT INTO "Produce"(
	VALUES ('{0}', '{1}', '{2}', '{3}'));
""".format(self.Produce_ID, self.Product_ID, self.Produce_Date)
    
    def edit_Produce(self):
        #pass

        print("--- >>>> Welcome to the Edit a Produce Section <<<< ---")
        edit_Prdc = input("please Enter the ID of the Produce that you want to Edit : ")
        cur = conn.cursor()
        st = """SELECT * FROM "Produce" WHERE "Produce_ID" = '{0}'""".format(edit_Prdc)
        cur.execute(st)
        a = cur.fetchone()
        print(a)
        if a == None:
            print("--->>>Error : There is No Produce with that Info. try Again --->")
            input_try = input("if you want to try Again Enter <<<1>>> : ")
            if input_try == '1':
                # print("hello")
                self.edit_Produce()
            ...
            if ed_ch == '1':
                ...
                while loop_check != '0':
                    print("---<<< Choose witch Item you want to Edit : >>>---")
                    print("1 --->>> Edit the <<Produce ID>> ---")
                    print("2 --->>> Edit the <<Product ID>> ---")
                    print("3 --->>> Edit the <<Produce Date>> ---")
                    print("0 --->>> to <<Exit>> ---")
                    loop_check = input("---->>>> Make your Choice <<<<--- : ")

                    if loop_check == '1':
                        edit_pci = input("please Enter the new Produce ID that you want : ")
                        st_2 = """UPDATE "Produce" SET "Produce_ID" = '{0}' WHERE "Produce_ID" = '{1}'
                        """.format(edit_pci, edit_Prdc)
                    
                    if loop_check == '2':
                        edit_pdi = input("please Enter the new Product ID that you want : ")
                        st_3 = """UPDATE "Produce" SET "Product_ID" = '{0}' WHERE "Product_ID" = '{1}'
                        """.format(edit_pdi, edit_Prdc)
                    
                    if loop_check == '3':
                        edit_pt = input("please Enter the new Produce Date that you want : ")
                        st_4 = """UPDATE "Produce" SET "Produce_Date" = '{0}' WHERE "Produce_Date" = '{1}'
                        """.format(edit_pt, edit_Prdc)
    ...
    def delete_Produce(self):
        #pass
        print("--- >>>> Welcome to the Delete a Produce Section <<<< ---")
        prod_del = input("please Enter the ID of the Produce that you want to Delete : ")
        st = """SELECT * FROM "Produce" WHERE "Produce_ID" = '{0}'""".format(prod_del)
        cur = conn.cursor()
        cur.execute(st)
        a = cur.fetchone()
        # print(a)
        if a == None:
            ...
            del_check = input("There is a Row with that info in the DB if you are Sure to delete Enter <<<1>>> --- > : ")
            if del_check == '1':
                st_2 = """DELETE FROM "Produce"  WHERE "Produce_ID" = '{0}'""".format(prod_del)
                cur.execute(st_2)
            else:
                pass
